...
const reservedWords = [
  ...
  'true', 'try', 'var', 'void', 'while', 'with', 'async', 'hide', 'on', 'show', 
  'sync', 'abstract', 'as', 'covariant', 'deferred', 'dynamic', 'export', 'extension', 
...
async function cloneIconSetsRepo() {
  await execShellCommand('git clone https://github.com/iconify/icon-sets.git generator/icon-sets');
}
...
async function getColoredIconsList() {
  const makeIcons = Object.entries(
    JSON.parse(await fs.readFile('generator/icon-sets/collections.json', 'utf8'))
  );
  const makeIconsList = [];
...
async function processIconset(iconset) {
  ...
  const input = await fs.readFile(`generator/icon-sets/json/${iconset}`, 'utf8');
...
async function createIconsetFile(outputPath, iconSetName, iconSetPrefix) {
  await fs.writeFile(
    outputPath,
    `///Discover all icons of this iconset at https://andronasef.ninja/iconify_flutter/collection/${iconSetPrefix} \n class ${iconSetName} {`
  );
}
...
async function processIcons(iconSet, outputPath, iconSetName) {
  ...
  await iconSet.forEach(async (name, type) => {
    ...
    await fs.appendFile(outputPath, `static const String ${iconName} = '${svg}';\n`);
  });
